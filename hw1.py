...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1, padding=padding) + b_conv1)
    h_pool1 = max_pool_2x2(h_conv1, padding=padding)

    # second layer
    W_conv2 = weight_variable([5, 5, 32, 64])
    b_conv2 = bias_variable([64])

    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2, padding=padding) + b_conv2)
    h_pool2 = max_pool_2x2(h_conv2, padding=padding)
    ...
    # readout layer
    W_fc2 = weight_variable([1024, outputsize])
    b_fc2 = bias_variable([outputsize])
